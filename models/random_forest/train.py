...
def main():
    ...
    X_test, y_test, _, _ = prepare_dataset(TEST_MAPPING, TEST_DIR)
    y_test = label_enc.transform(
        [class_names[i] if i < len(class_names) else '' for i in y_test]
    ) if len(set(y_test)) > 0 else y_test

    # Re-encode test labels with same encoder
    from utils.data_loader import load_fragments
