...
OUT_TRAIN = os.path.join(BASE_DIR, 'fragments_headerless_footerless', 'train_fragments')
OUT_TEST = os.path.join(BASE_DIR, 'fragments_headerless_footerless', 'test_fragments')
...
FOOTER_TRIM = 0           # Set to >0 if you want to trim bytes from the end
...
def generate_fragments(src_dir, out_dir):
    ...
    for fname in os.listdir(src_dir):
        ...
        data = data[HEADER_SIZE:len(data)-FOOTER_TRIM if FOOTER_TRIM > 0 else None]
        file_size = len(data)
        print(f"File size after header/footer removal: {file_size} bytes")
